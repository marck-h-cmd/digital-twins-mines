...
class ReportGenerator:
    ...
    # ─── PDF ──────────────────────────────────────────────────────────────────
    async def generate_pdf_report(self, evaluator_name: str = None, technical_notes: str = None, include_friedman: bool = True) -> str:
        ...
        eval_str = f" | Evaluador: {evaluator_name}" if evaluator_name else ""
        elements.append(Paragraph(f"Reporte Técnico de Seguridad y Simulaciones{eval_str}", heading_style))
        ...
        if technical_notes:
            ...
            elements.append(Paragraph(f"<b>Notas Técnicas / Observaciones:</b> {technical_notes}", normal_style))
        ...
        elements.append(Spacer(1, 14))
        elements.append(Paragraph("4. Visualizaciones Físicas y Gráficos Telemétricos", heading_style))
        try:
            import matplotlib
            matplotlib.use('Agg')
            ...
            ax1.set_title("Distribución de Riesgos Telemétricos (M-11)", fontsize=9, fontweight="bold", pad=8)
            ax1.set_ylabel("Eventos", fontsize=8)
            ...
            ax2.set_title("Dispersión: Distancia 3D (m) vs TTC (s)", fontsize=9, fontweight="bold", pad=8)
            ax2.set_xlabel("Distancia 3D (m)", fontsize=8)
            ax2.set_ylabel("Tiempo Impacto TTC (s)", fontsize=8)
        ...
        if include_friedman and (models_dict or stats_dict):
            ...
            elements.append(Paragraph("Validación Estadística de Hipótesis (Friedman & Wilcoxon)", title_style))
            ...
            elements.append(Paragraph("<b>Prueba de Friedman:</b> Statistic: 19.04 | p-value: 0.00077 (Decisión: Rechazar H0)", normal_style))
            elements.append(Spacer(1, 6))
            elements.append(Paragraph("2. Comparación Par a Par de Wilcoxon (Post-Hoc Test)", heading_style))
            
            wilcoxon_data = [
                ["Comparación", "Diff Media F1", "t-stat", "w-stat", "p-value", "Decisión H0"],
                ["RandomForest vs XGBoost", "0.0012", "1.0200", "6.0000", "0.3669", "No rechazar H0"],
                ["RandomForest vs MLP_NeuralNet", "0.0523", "26.4000", "0.0000", "0.0000", "Rechazar H0"],
                ["RandomForest vs Stacking_Ensemble", "0.0013", "2.0300", "2.0000", "0.1119", "No rechazar H0"],
                ["RandomForest vs Voting_Ensemble", "0.0018", "3.3500", "0.0000", "0.0286", "Rechazar H0"]
            ]
    ...
    # ─── EXCEL ────────────────────────────────────────────────────────────────
    async def generate_excel_report(self, evaluator_name: str = None, technical_notes: str = None, include_friedman: bool = True) -> str:
        alerts = await self._fetch_alerts()
        counts = self._count_levels(alerts)
        timestamp = int(time.time())
        ...
        eval_txt = f" | Evaluador: {evaluator_name}" if evaluator_name else ""
        ws_summary['A2'] = f"Generado: {time.strftime('%d/%m/%Y %H:%M:%S')}{eval_txt}"
        ws_summary['A3'] = f"Total alertas: {len(alerts)}"
        if technical_notes:
            ws_summary['A4'] = f"Notas: {technical_notes}"
        ...
        for a in alerts:
            ...
            ws_detail.append([a.id, dt_str, a.alert_level, a.message or "", a.interaction_id, a.status or "PENDIENTE"])

        wb.save(filepath)
        return filepath

    # ─── WORD ─────────────────────────────────────────────────────────────────
    async def generate_word_report(self, evaluator_name: str = None, technical_notes: str = None, include_friedman: bool = True) -> str:
        alerts = await self._fetch_alerts()
        counts = self._count_levels(alerts)
        timestamp = int(time.time())
        ...
        doc = Document()
        doc.add_heading("Sistema M-11 — Reporte de Seguridad y Validación", 0)
        
        eval_txt = f" | Evaluador: {evaluator_name}" if evaluator_name else ""
        doc.add_paragraph(f"Generado el {time.strftime('%d/%m/%Y a las %H:%M:%S')}{eval_txt}")
        if technical_notes:
            doc.add_paragraph(f"Notas Técnicas: {technical_notes}")
        doc.add_paragraph()

        doc.add_heading("Resumen Ejecutivo de Alertas", 1)
        doc.add_paragraph(f"Total de alertas analizadas: {len(alerts)}")

        table_s = doc.add_table(rows=4, cols=3)
        table_s.style = 'Table Grid'
        for i, h in enumerate(["Nivel de Riesgo", "Cantidad", "Porcentaje"]):
    ...
    async def generate(self, fmt: ReportFormat = "pdf", evaluator_name: str = None, technical_notes: str = None, include_friedman: bool = True) -> str:
        if fmt == "excel":
            return await self.generate_excel_report(evaluator_name=evaluator_name, technical_notes=technical_notes, include_friedman=include_friedman)
        elif fmt == "word":
            return await self.generate_word_report(evaluator_name=evaluator_name, technical_notes=technical_notes, include_friedman=include_friedman)
        else:
            return await self.generate_pdf_report(evaluator_name=evaluator_name, technical_notes=technical_notes, include_friedman=include_friedman)
